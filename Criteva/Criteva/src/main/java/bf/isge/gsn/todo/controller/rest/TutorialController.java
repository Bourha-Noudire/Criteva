...
public class TutorialController {
  ...
  public String saveTutorial(Tutorial tutorial, RedirectAttributes redirectAttributes) {
    try {
      ...
      redirectAttributes.addFlashAttribute("message", "Critère ajouté avec succès !!!");
    } catch (Exception e) {
  ...
  public String editTutorial(@PathVariable("id") Integer id, Model model, RedirectAttributes redirectAttributes) {
    try {
      ...
      model.addAttribute("tutorial", tutorial);
      model.addAttribute("pageTitle", "Modifier le critère (ID: " + id + ")");
  ...
  public String deleteTutorial(@PathVariable("id") Integer id, Model model, RedirectAttributes redirectAttributes) {
    try {
      ...
      redirectAttributes.addFlashAttribute("message", "Le citère avec l'id=" + id + " a été supprimé avec succès !!!");
    } catch (Exception e) {
      redirectAttributes.addFlashAttribute("message", e.getMessage());
    }
  ...
  /*@GetMapping("/tutorials/{id}/evaluation/{score}")
  public String evaluateTutorial(@PathVariable("id") Integer id, @PathVariable("score") Integer score,
                                 Model model, RedirectAttributes redirectAttributes) {
    try {
      tutorialRepository.updateEvaluationScore(id, score);

      String message = "Le critère id=" + id + " a été evalué avec le score de " + score;
